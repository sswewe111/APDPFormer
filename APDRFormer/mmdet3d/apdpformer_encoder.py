...
from mmdet3d.models.utils.disentangle import DisentangleMoudule
from mmdet3d.models.utils.fusionmodule import LRF
from .bev_fusion import SFA
...
class APDPFormerEncoder(BaseModule):
    ...
    def __init__(self, num_points_in_pillar=10, embed_dims=256, num_classes=10, **kwargs):
        ...
        # instance fusion
        self.instance_num = kwargs.get('instance_num', 100)
        self.instance_num_img = kwargs.get('instance_num_img', 50)
        self.instance_num_pts = kwargs.get('instance_num_pts', 50)
        ...
        self.instance_att_img = InsContextAtt(num_layers=2, embed_dims=embed_dims // 2, bev_size=self.bev_size)
        self.instance_att_pts = InsContextAtt(num_layers=2, embed_dims=embed_dims // 2, bev_size=self.bev_size)

        self.disentangle = DisentangleMoudule(embed_dims // 2, embed_dims // 2)
        self.fusion_layer = LRF([embed_dims // 2, embed_dims // 2], embed_dims // 2)
        self.bev_fusion = SFA(in_channels=128, out_channels=128, num_classes=10, q=5, hidden_channels=64)
        ...
        self.conv_scene_img = ConvModule(
            embed_dims // 2,
            embed_dims // 2,
            kernel_size=3,
            padding=1,
        ...
        self.conv_scene_pts = ConvModule(
            embed_dims // 2,
            embed_dims // 2,
            kernel_size=3,
            padding=1,
        ...
        self.heatmap_head_1_img = ConvModule(
            embed_dims // 2,
            embed_dims // 4,
            kernel_size=3,
            padding=1,
        ...
        self.heatmap_head_2_img = ConvModule(
            embed_dims // 4,
            embed_dims // 4,
            kernel_size=3,
            padding=1,
        ...
        self.heatmap_head_3_img = nn.Conv2d(embed_dims // 4, num_classes, kernel_size=3, stride=1, padding=1)

        self.heatmap_head_1_pts = ConvModule(
            embed_dims // 2,
            embed_dims // 4,
            kernel_size=3,
            padding=1,
        ...
        self.heatmap_head_2_pts = ConvModule(
            embed_dims // 4,
            embed_dims // 4,
            kernel_size=3,
            padding=1,
        ...
        self.heatmap_head_3_pts = nn.Conv2d(embed_dims // 4, num_classes, kernel_size=3, stride=1, padding=1)
    ...
    def instance_fusion(self, bev_feats, scene_feats, bs, **kwargs):
        img = kwargs.get('img_bev_feats', None)
        pts = kwargs.get('lidar_feats', None)
        ...
        else:  # for Pedestrian & Cyclist in KITTI
        ...
        # query_pos_new=query_pos

        acc_masks = torch.ones_like(ins_heatmap).view(bs, -1)  # [B,C,W,H] -> [B,C*W*H]
        selected_mask = acc_masks.new_zeros(bs, self.num_classes * ins_heatmap.shape[-2] * ins_heatmap.shape[
            -2])  # [B,num_classes*W*H]
        selected_mask.scatter_(index=top_proposals, dim=1,
                               src=torch.ones_like(top_proposals, dtype=acc_masks.dtype))

        selected_mask = selected_mask.reshape(*ins_heatmap.shape)
        # masking by pooling
        selected_mask_kernel = F.max_pool2d(selected_mask, kernel_size=self.nms_kernel_size, stride=1,
                                            padding=self.nms_kernel_size // 2)
        if self.num_views == 6:  ## for Pedestrian & Traffic_cone in nuScenes
            selected_mask_kernel[:, 8:10] = F.max_pool2d(selected_mask[:, 8:10], kernel_size=1, stride=1,
                                                         padding=0)
        else:  # for Pedestrian & Cyclist in KITTI
            selected_mask_kernel[:, 1:3] = F.max_pool2d(selected_mask[:, 1:3], kernel_size=1, stride=1,
                                                        padding=0)
        acc_masks = acc_masks * (1. - selected_mask_kernel).view(*acc_masks.shape)

        ins_heatmap_img = kwargs.get('ins_heatmap_img', None)
        ins_heatmap_pts = kwargs.get('ins_heatmap_pts', None)

        acc_masks_img = acc_masks.clone()
        acc_masks_pts = acc_masks.clone()
        heatmap_img = ins_heatmap_img.detach().sigmoid()
        heatmap_img = heatmap_img * acc_masks_img.view(*heatmap_img.shape)
        heatmap_pts = ins_heatmap_pts.detach().sigmoid()
        heatmap_pts = heatmap_pts * acc_masks_pts.view(*heatmap_pts.shape)

        multistage_masks = []
        # multistage_masks.append(acc_masks_img.view(*heatmap_img.shape).permute(0, 1, 3, 2).clone())
        # multistage_masks.append(acc_masks_pts.view(*heatmap_pts.shape).permute(0, 1, 3, 2).clone())
        multistage_masks.append(acc_masks_img.view(*heatmap_img.shape).clone())
        multistage_masks.append(acc_masks_pts.view(*heatmap_pts.shape).clone())

        local_max_img = torch.zeros_like(heatmap_img)
        local_max_pts = torch.zeros_like(heatmap_pts)
        # equals to nms radius = voxel_size * out_size_factor * kenel_size
        local_max_inner_img = F.max_pool2d(heatmap_img, kernel_size=self.nms_kernel_size, stride=1, padding=0)
        local_max_inner_pts = F.max_pool2d(heatmap_pts, kernel_size=self.nms_kernel_size, stride=1, padding=0)
        local_max_img[:, :, padding:(-padding), padding:(-padding)] = local_max_inner_img
        local_max_pts[:, :, padding:(-padding), padding:(-padding)] = local_max_inner_pts
        ## for Pedestrian & Traffic_cone in nuScenes
        if self.num_views == 6:
            local_max_img[:, 8, ] = F.max_pool2d(heatmap_img[:, 8], kernel_size=1, stride=1, padding=0)
            local_max_img[:, 9, ] = F.max_pool2d(heatmap_img[:, 9], kernel_size=1, stride=1, padding=0)
            local_max_pts[:, 8, ] = F.max_pool2d(heatmap_pts[:, 8], kernel_size=1, stride=1, padding=0)
            local_max_pts[:, 9, ] = F.max_pool2d(heatmap_pts[:, 9], kernel_size=1, stride=1, padding=0)
        else:  # for Pedestrian & Cyclist in KITTI
            local_max_img[:, 1, ] = F.max_pool2d(heatmap_img[:, 1], kernel_size=1, stride=1, padding=0)
            local_max_img[:, 2, ] = F.max_pool2d(heatmap_img[:, 2], kernel_size=1, stride=1, padding=0)
            local_max_pts[:, 1, ] = F.max_pool2d(heatmap_pts[:, 1], kernel_size=1, stride=1, padding=0)
            local_max_pts[:, 2, ] = F.max_pool2d(heatmap_pts[:, 2], kernel_size=1, stride=1, padding=0)
        heatmap_img = heatmap_img * (heatmap_img == local_max_img)
        heatmap_img = heatmap_img.view(bs, heatmap_img.shape[1], -1)
        heatmap_pts = heatmap_pts * (heatmap_pts == local_max_pts)
        heatmap_pts = heatmap_pts.view(bs, heatmap_pts.shape[1], -1)

        instance_num_img = self.instance_num_img
        instance_num_pts = self.instance_num_pts
        top_proposals_img = heatmap_img.view(bs, -1).argsort(dim=-1, descending=True)[..., : instance_num_img]
        top_proposals_pts = heatmap_pts.view(bs, -1).argsort(dim=-1, descending=True)[..., : instance_num_pts]

        top_proposals_index_img = top_proposals_img % heatmap_img.shape[-1]
        top_proposals_index_pts = top_proposals_pts % heatmap_pts.shape[-1]
        query_pos_img = bev_pos.gather(
            index=top_proposals_index_img[:, None, :].permute(0, 2, 1).expand(-1, -1, bev_pos.shape[-1]),
            dim=1)
        query_pos_pts = bev_pos.gather(
            index=top_proposals_index_pts[:, None, :].permute(0, 2, 1).expand(-1, -1, bev_pos.shape[-1]),
            dim=1)

        query_pos_new_img = torch.zeros_like(query_pos_img)
        query_pos_new_img[..., 0] = query_pos_img[..., 1]
        query_pos_new_img[..., 1] = query_pos_img[..., 0]
        # query_pos_new_img=query_pos_img

        query_pos_new_pts = torch.zeros_like(query_pos_pts)
        query_pos_new_pts[..., 0] = query_pos_pts[..., 1]
        query_pos_new_pts[..., 1] = query_pos_pts[..., 0]
        # query_pos_new_pts=query_pos_pts

        x_scene = self.conv_scene(bev_feats.permute(0, 1, 3, 2))
        x_scene_flatten = x_scene.view(bs, x_scene.shape[1], -1)
        x_ins = x_scene_flatten.gather(index=top_proposals_index[:, None, :].expand(-1, x_scene.shape[1], -1), dim=-1)

        x_scene_img = self.conv_scene_img(img)
        x_scene_flatten_img = x_scene_img.view(bs, x_scene_img.shape[1], -1)
        x_ins_img = x_scene_flatten_img.gather(
            index=top_proposals_index_img[:, None, :].expand(-1, x_scene_img.shape[1], -1), dim=-1)

        x_scene_pts = self.conv_scene_pts(pts)
        x_scene_flatten_pts = x_scene_pts.view(bs, x_scene_pts.shape[1], -1)
        x_ins_pts = x_scene_flatten_pts.gather(
            index=top_proposals_index_pts[:, None, :].expand(-1, x_scene_pts.shape[1], -1), dim=-1)


        x_ins = self.instance_att(x_ins, query_pos_new, bev_pos, scene_feats=x_scene, **kwargs)
        x_ins_img = self.instance_att_img(x_ins_img, query_pos_new_img, bev_pos, scene_feats=x_scene_img, **kwargs)
        x_ins_pts = self.instance_att_pts(x_ins_pts, query_pos_new_pts, bev_pos, scene_feats=x_scene_pts, **kwargs)

        x_ins = self.align_fusion(x_ins.permute(0, 2, 1)).permute(0, 2, 1)
        x_ins_img = self.align_img(x_ins_img.permute(0, 2, 1)).permute(0, 2, 1)
        x_ins_pts = self.align_pts(x_ins_pts.permute(0, 2, 1)).permute(0, 2, 1)

        x_ins_sum = []
        x_ins_sum.append(x_ins)
        x_ins_sum.append(x_ins_img)
        x_ins_sum.append(x_ins_pts)
        x_ins = torch.cat(x_ins_sum, dim=2)
        ...
        return return_features, ins_heatmap, ins_heatmap_img, ins_heatmap_pts, multistage_masks
    ...
    def forward(self,
        ...
        img_bev_feats = self.shared_conv_img(img_bev_feats)
        img_spec, pts_spec, share_spec = self.disentangle(img_bev_feats, lidar_feats)

        img_spec = img_spec.permute(0, 1, 3, 2).contiguous()
        pts_spec = pts_spec.permute(0, 1, 3, 2).contiguous()

        out_img = share_spec[0].permute(0, 1, 3, 2).contiguous()
        out_pts = share_spec[1].permute(0, 1, 3, 2).contiguous()


        ins_heatmap_img = self.heatmap_head_1_img(out_img.clone().detach())
        ins_heatmap_img = self.heatmap_head_2_img(ins_heatmap_img)
        ins_heatmap_img = self.heatmap_head_3_img(ins_heatmap_img)

        ins_heatmap_pts = self.heatmap_head_1_pts(out_pts.clone().detach())
        ins_heatmap_pts = self.heatmap_head_2_pts(ins_heatmap_pts)
        ins_heatmap_pts = self.heatmap_head_3_pts(ins_heatmap_pts)
        kwargs.update(dict(ins_heatmap_img=ins_heatmap_img))
        kwargs.update(dict(ins_heatmap_pts=ins_heatmap_pts))

        kwargs.update(dict(img_bev_feats=out_img))
        kwargs.update(dict(lidar_feats=out_pts))

        # bev_feats = self.fusion_layer(share_spec)
        bev_feats = self.bev_fusion(share_spec[1], share_spec[0], ins_heatmap_pts, ins_heatmap_img)        
        ...
        for i in range(len(self.get_regions)):
            ...
            if i == 0:
                x[0], ins_hm, ins_heatmap_img, ins_heatmap_pts, multistage_masks = self.instance_fusion(bev_feats, x[0],
                                                                                                        bs, **kwargs)

            grid_features, grid_coords, this_feat = pts_backbone(x, 'stage{}'.format(i + 1))
            return_feats.append(this_feat)

        return return_feats, img_spec, pts_spec, ins_hm, ins_heatmap_img, ins_heatmap_pts, multistage_masks
