...
from mmdet3d.models.builder import HEADS, build_loss,MODELS
...
from mmdet3d.models.utils.lightattention import lightMMatten
...
class TransFusionHeadV3(nn.Module):
    def __init__(self,
                 num_proposals=128,
                 auxiliary=True,
                 in_channels=128 * 3,
        ...
        super(TransFusionHeadV3, self).__init__()
        ...
        self.loss_heatmap_img = build_loss(loss_heatmap)
        self.loss_heatmap_pts = build_loss(loss_heatmap)
        ...
        self.aux_heatmaphead_2d = nn.Sequential(*layers)

        self.specific_fusion = lightMMatten()

        self.img_fuse = MODELS.build(
            img_fuse) if img_fuse is not None else None

        self.pts_fuse = MODELS.build(
            pts_fuse) if pts_fuse is not None else None
    ...
    def forward_single(self, inputs,  metas):
        ...
        fusion_feat = self.shared_conv(fused_feat)
        #################################
        fusion_feat_flatten = fusion_feat.view(batch_size,
                                               fusion_feat.shape[1],
                                               -1).contiguous()  # [BS, C, H*W]
        img_feat_flatten = img_feat.view(batch_size,
                                         img_feat.shape[1],
                                         -1).contiguous()  # [BS, C, H*W]

        pts_feat_flatten = pts_feat.view(batch_size,
                                         pts_feat.shape[1],
                                         -1).contiguous()  # [BS, C, H*W]
        ...
        elif self.test_cfg["dataset"] == "KITTI":  # for Pedestrian & Cyclist in KITTI
        ...
        query_pos = bev_pos.gather(
            index=top_proposals_index[:, None, :].permute(0, 2, 1).contiguous().expand(
                -1, -1, bev_pos.shape[-1]),
            dim=1,
        )
        ...
        img_query_feat = img_feat_flatten.gather(
            index=top_proposals_index[:, None, :].expand(
                -1, img_feat_flatten.shape[1], -1),
            dim=-1,
        )

        pts_query_feat = pts_feat_flatten.gather(
            index=top_proposals_index[:, None, :].expand(
                -1, pts_feat_flatten.shape[1], -1),
            dim=-1,
        )
        ...
        cls_query_feat, bbox_query_feat = self.specific_fusion(torch.cat([img_query_feat, pts_query_feat], dim=-1),self.num_proposals)
        ###################################
        # task preference sup
        ###################################
        aux_res_layer = dict()
        for task in ['height', 'center', 'dim', 'rot','vel']:
            aux_res_layer['aux_{}'.format(task)] = \
                self.aux_prediction_heads[0].__getattr__(task)(bbox_query_feat)
        aux_res_layer['aux_center'] += query_pos.permute(0, 2, 1).contiguous()

        aux_res_layer['aux_heatmap'] = \
            self.aux_prediction_heads[0].__getattr__('heatmap')(cls_query_feat)

        with torch.autocast('cuda', enabled=False):
            aux_denseheatmap_2d = self.aux_heatmaphead_2d(img_feat.float())

        aux_res_layer['aux_dsheatmp_2d'] = aux_denseheatmap_2d
        ...
        for i in range(self.num_decoder_layers):
            ...
            query_feat = self.decoder[i](
                query_feat, fusion_feat_flatten, query_pos, bev_pos
            )
            cls_query_feat = self.img_fuse([cls_query_feat, query_feat])

            bbox_query_feat = self.pts_fuse([bbox_query_feat, query_feat])
            # Prediction
            res_layer = dict()
            for task in ['height', 'center', 'dim', 'rot','vel']:
                res_layer[task] = \
                    self.prediction_heads[i].__getattr__(task)(bbox_query_feat)
            res_layer['center'] += query_pos.permute(0, 2, 1).contiguous()

            res_layer['heatmap'] = \
                self.prediction_heads[i].__getattr__('heatmap')(cls_query_feat)
        ...
        final_res = dict()
        final_res.update(new_res)
        final_res.update(aux_res_layer)
    ...
    def forward(self, feats, img_feats, metas):
        ...
        if img_feats is None:
            img_feats = [None]
        res = self.forward_single(feats, [metas])
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, ins_heatmap=None,ins_heatmap_img=None,ins_heatmap_pts=None,multistage_masks=None, **kwargs):
        ...
        if multistage_masks is not None:
            multistage_mask=multistage_masks
           
            
            heatmap_ins_img = heatmap 
            heatmap_ins_pts = heatmap 
            

            if ins_heatmap_img is not None:
                loss_ins_img = self.loss_heatmap_img(
                    clip_sigmoid(ins_heatmap_img),
                    heatmap_ins_img,
                    # weight=multistage_mask[0],
                    avg_factor=max(heatmap_ins_img.eq(1).float().sum().item(), 1))
                loss_dict["loss_heatmap_img"] = loss_ins_img

            if ins_heatmap_pts is not None:
                loss_ins_pts = self.loss_heatmap_pts(
                    clip_sigmoid(ins_heatmap_pts),
                    heatmap_ins_pts,
                    # weight=multistage_mask[1],
                    avg_factor=max(heatmap_ins_pts.eq(1).float().sum().item(), 1))
                loss_dict["loss_heatmap_pts"] = loss_ins_pts
        ...
        aux_preds = dict()
        for key in ['height', 'center', 'dim', 'rot','vel', 'heatmap']:
            aux_preds[key] = preds_dicts[0][0][f'aux_{key}']
        (
            aux_labels,
            aux_label_weights,
            aux_bbox_targets,
            ...
            aux_matched_ious,
            aux_heatmap,
        ) = self.get_targets(gt_bboxes_3d, gt_labels_3d, [aux_preds])
        aux_layer_labels = aux_labels.reshape(-1)
        aux_layer_label_weights = aux_label_weights.reshape(-1)
        aux_layer_score = aux_preds['heatmap']
        aux_layer_cls_score = aux_layer_score.permute(0, 2, 1).reshape(-1, self.num_classes)
        aux_layer_loss_cls = self.loss_cls(
            aux_layer_cls_score.float(),
            aux_layer_labels,
            aux_layer_label_weights,
            #avg_factor=max(aux_num_pos, 1),
            avg_factor=max(num_pos, 1),
        )
        if 'vel' in aux_preds.keys():
            auxpred = torch.cat(
                [aux_preds['center'], aux_preds['height'], aux_preds['dim'], aux_preds['rot'], aux_preds['vel']],
                dim=1).permute(0, 2, 1)  # [BS, num_proposals, code_size]
        else:
            auxpred = torch.cat(
                [aux_preds['center'], aux_preds['height'], aux_preds['dim'], aux_preds['rot']],
                dim=1).permute(0, 2, 1)  # [BS, num_proposals, code_size]
        code_weights = self.train_cfg.get('code_weights', None)
        aux_layer_bbox_weights = aux_bbox_weights
        aux_layer_reg_weights = aux_layer_bbox_weights * aux_layer_bbox_weights.new_tensor(  # noqa: E501
            code_weights)
        aux_layer_bbox_targets = aux_bbox_targets
        aux_layer_loss_bbox = self.loss_bbox(
            auxpred,
            aux_layer_bbox_targets,
            aux_layer_reg_weights,
            avg_factor=max(aux_num_pos, 1))
        loss_heatmap_2d = self.loss_heatmap(
            clip_sigmoid(preds_dict['aux_dsheatmp_2d']).float(),
            heatmap.float(),
            #avg_factor=max(aux_heatmap.eq(1).float().sum().item(), 1),
            avg_factor=max(heatmap.eq(1).float().sum().item(), 1),
        )

        loss_dict['aux_heatmap_2d_loss'] = loss_heatmap_2d

        loss_dict['aux_loss_cls'] = aux_layer_loss_cls
        loss_dict['aux_loss_bbox'] = aux_layer_loss_bbox
        loss_dict['aux_matched_ious'] = aux_layer_loss_cls.new_tensor(aux_matched_ious)
        ...
        for layer_id, preds_dict in enumerate(preds_dicts):
            ...
            if self.test_cfg["dataset"] == "nuScenes":
                ...
                self.tasks = [
                    ...
                    dict(
                        num_class=1, class_names=["Pedestrian"], indices=[1], radius=0.5
                    ),
                    dict(num_class=1, class_names=["Cyclist"], indices=[2], radius=0.5),
                ]
